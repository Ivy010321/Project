...
class 人():
    def __init__(self, i):
        #这个def写的是一开始的人们（不包括最初被发现新馆疫情的人）
        #在这个def中的阶段，还没有人们被传染
        #定义了没个区域得大小和每个区域可以容纳的人数
        #定义了每个人得movement
        self.uid = i
        ...
        #给出每个常住区域会出现的数量范围（用了random语句：在3~6之间随机选择一个数）
        区域分布列表[self.x][self.y].append(self)
        #区域分布的x和y的list里头加入self的（新的dots）
        self.常住区域.add((self.x, self.y))
        #把self的dots的x，y坐标加入到常住区域里
        self.已感染 = False
        self.传染性 = False
        self.免疫 = random.random() < 免疫概率
        self.已入院 = False
        self.已进方舱 = False
        #已感染、已入院、已进方舱、传染性在健康的人身上是不存在的，而这个def写的是健康的人的movement
        #所以以上的几个都是False的
        self.身体状况 = '健康'
        #对这个def里的每个人的身体状况加入描述
        self.感染后天数 = 0
        self.潜伏期 = 0
        self.发病期 = 0
        self.传染期 = 0
        #def所描述的是健康的人，所以以上关于新馆疫情的数据都为0
    def 随机位置(self):
        # 80%人分布在25%区域
        L = int(0.25 * 区域大小)
